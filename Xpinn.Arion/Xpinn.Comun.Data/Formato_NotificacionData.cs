        ...
        public Formato_Notificacion ObtenerFormatoAEnviar(Formato_Notificacion noti, Usuario usuario)
        {
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(usuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = @"select * from Formatodocumentoscorreo where cod_documento = " + noti.codigo+ "";
                        connection.Open();
                        cmdTransaccionFactory.Connection = connection;
                        cmdTransaccionFactory.CommandType = CommandType.Text;
                        cmdTransaccionFactory.CommandText = sql;
                        ...
                        if (resultado.Read())
                        {
                            if (resultado["DESCRIPCION"] != DBNull.Value) noti.nombre = Convert.ToString(resultado["NOMBRE"]).Trim();
