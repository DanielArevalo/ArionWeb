        ...
        public List<Acceso> ListarOpciones(Int64 IdPerfil, Int64 CodModulo, Usuario vUsuario)
        {
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = @"SELECT Perfil_Acceso.codperfil, Opciones.cod_opcion, Opciones.nombre, Perfil_Acceso.consultar, Perfil_Acceso.insertar, Perfil_Acceso.modificar, Perfil_Acceso.borrar, Opciones.cod_proceso
