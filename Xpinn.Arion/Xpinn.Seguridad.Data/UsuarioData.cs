                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = @"SELECT PA.COD_PERSONA,V.IDENTIFICACION,trim(PA.NOMBRES)||' '||trim(PA.APELLIDOS) as NOMBRE, V.DIRECCION, V.TELEFONO, PA.CLAVE, PA.FECHA_CREACION AS FECHACREACION, 
                                        ...
                                        WHERE V.IDENTIFICACION = '" + pUsuario.Trim() + "' AND PA.CLAVE = '" + pClave.Trim() + "'";
        ...
        public Perfil ConsultarFechaperiodicidad(Int64 CodUsuario, Int64 CodPerfil, Usuario vUsuario)
        {
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    ...
                    string sql2 = @"SELECT P.COD_PERIODICIDAD as COD_PERIODICIDAD ,T.NUMERO_DIAS as NUMERO_DIAS
                                         FROM USUARIOS U INNER JOIN PERFIL_USUARIO P ON U.CODPERFIL = P.CODPERFIL
                                         LEFT JOIN PERIODICIDAD T ON T.COD_PERIODICIDAD = P.COD_PERIODICIDAD                                        
                                         WHERE U.CODUSUARIO = " + CodUsuario + " AND P.CODPERFIL = " + CodPerfil;
