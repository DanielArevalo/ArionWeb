...
namespace Xpinn.Seguridad.Data
{
    ...
    public class AccesoData : GlobalData
    {
        ...
        public List<int> ListarAccesoApp(Usuario pUsuario)
        {
            DbDataReader resultado = default(DbDataReader);
            List<int> lstAcceso = new List<int>();
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(pUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = @"SELECT distinct(opciones.cod_opcion), Opciones.Nombre
                                       FROM perfil_acceso
                                       INNER JOIN opciones ON perfil_acceso.cod_opcion = opciones.cod_opcion
                                       INNER JOIN proceso ON proceso.cod_proceso = opciones.cod_proceso
                                       INNER JOIN modulo ON modulo.cod_modulo = proceso.cod_modulo
                                       WHERE Perfil_Acceso.Consultar = 1
                                       AND modulo.cod_modulo = 29
                                       order by 1";
                        ...
                        {
                            int codigo = 0;
                            if (resultado["COD_OPCION"] != DBNull.Value) codigo = Convert.ToInt32(resultado["COD_OPCION"]);
                            if(codigo > 0)
                                lstAcceso.Add(codigo);
                        }
                        return lstAcceso;
                    }
                    catch (Exception ex)
