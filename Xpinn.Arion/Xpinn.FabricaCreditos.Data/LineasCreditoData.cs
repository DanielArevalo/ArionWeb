                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        string sql = @"select L.TASA
                                       ...
                                       where A.COD_LINEA_CREDITO = " + cod_linea_credito + @"and
                                       (T.TIPO_TOPE = 2 AND L.TASA IS NOT NULL AND T.MINIMO <= " + plazo + " AND T.MAXIMO >= " + plazo + ")";
            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(pUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        sql = @"Select Distinct l.cod_linea_credito, l.nombre, l.tipo_liquidacion, max(a.tasa) tasa, a.tipo_tasa, t.nombre as descripcion_tipotasa, l.cuotas_extras, d.cod_linea_credito AFIANCOL
                                        ...
                                        Group by l.cod_linea_credito, l.nombre, l.tipo_liquidacion, a.tipo_tasa, t.nombre, l.cuotas_extras, d.cod_linea_credito
                                        Order by 2";
