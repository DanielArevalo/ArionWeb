            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sql = "";
                        sql = "Select d.cod_cuenta, d.nomcue, d.naturaleza, d.tercero, d.identificacion, d.tipo_iden, t.descripcion, d.nom_tercero, d.tipo_persona, '' as Regimen, e.fecha, e.num_comp, e.tipo_comp, d.detalle, d.tipo, d.valor, 0 As saldo, e.concepto, 'P' As tipo_benef, e.N_DOCUMENTO As num_sop, d.centro_costo " +
                        ...
                        {                            
                            ...
                            sql = @"Select Distinct p.cod_cuenta, p.nombre As nomcue, p.tipo, b.cod_ter As tercero, Case tipo_persona When 'N' Then Trim(Substr(primer_nombre || ' ' || segundo_nombre || ' ' || primer_apellido || ' ' || segundo_apellido, 0, 240)) Else razon_social End As nom_tercero, c.identificacion
                        ...
                        else
                        {
                            ...
                            sql = @"Select Distinct p.cod_cuenta, p.nombre As nomcue, p.tipo, b.cod_ter As tercero, Decode(tipo_persona, 'N', Trim(Substr(primer_nombre || ' ' || segundo_nombre || ' ' || primer_apellido || ' ' || segundo_apellido, 0, 240)), razon_social) As nom_tercero, c.identificacion
                                From plan_cuentas p Left Join balance_ter b On b.cod_cuenta = p.cod_cuenta " + condicionfecha + @" And (b.saldo_ini != 0 Or b.saldo_fin != 0)
                                Left Join v_persona c On b.cod_ter = c.cod_persona
                                Where p.maneja_ter = 1 ";
                            if (CodCueIni.Trim() != "")
