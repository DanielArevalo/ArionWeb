                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        if (pEntidad.solonivel == 1)
                            ...
                            sqltot = "Select Sum(debito) As debito, Sum(credito) As credito from TEMP_LIBROMAYOR Where fecha = To_Date('" + pEntidad.fecha_corte.ToShortDateString() + "','" + conf.ObtenerFormatoFecha() + "') And nivel = 3";
