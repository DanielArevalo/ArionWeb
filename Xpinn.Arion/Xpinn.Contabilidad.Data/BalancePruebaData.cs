            ...
            using (DbConnection connection = dbConnectionFactory.ObtenerConexion(vUsuario))
            {
                ...
                using (DbCommand cmdTransaccionFactory = dbConnectionFactory.dbProveedorFactory.CreateCommand())
                {
                    try
                    {
                        ...
                        string sqltot = "";                       
                        sqltot = "Select fecha, Sum(debito) As debito, Sum(credito) As credito, Calcular_Utilidad(fecha, (Select Min(centro_costo) From centro_costo), (Select Max(centro_costo) From centro_costo)) As Utilidad From TEMP_LIBROMAYOR Where nivel = 1 And fecha = To_Date('" + Convert.ToDateTime(pEntidad.fecha).ToShortDateString() + "','" + conf.ObtenerFormatoFecha() + "') Group by fecha"; 
                        ...
                        sqltot = @"Select Nvl(Calcular_Utilidad(t.fecha, " + pEntidad.centro_costo.ToString() + @", " + pEntidad.centro_costo_fin.ToString() + @"), 0) -
                                    Nvl(Calcular_Utilidad((Select Max(c.fecha) From cierea c Where tipo = 'C' And c.fecha < t.fecha), " + pEntidad.centro_costo.ToString() + @", " + pEntidad.centro_costo_fin.ToString() + @"), 0) As Utilidad  
